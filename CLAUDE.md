...
- `static/favicon.svg`, `favicon.ico`, `apple-touch-icon.png` — иконки сайта; генерирует `brand/build.py` репозитория `delosvod` (знак семьи Делосвода: список с точкой-статусом, акцент `#3498DB`) — руками не править, знак в `<h1>` продублирован инлайном там же.
