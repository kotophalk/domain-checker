...
def check_domain(domain):
    try:
        ...
        return {"domain": domain, "free": q is None, "error": None}
    except Exception as e:
        # If the whois query throws an exception, it could be an
        # unsupported TLD or connection error
        error_msg = str(e)
        if "Unknown TLD" in error_msg:
            return {
                "domain": domain,
                "free": False,
                "error": "Неподдерживаемая зона",
            }
...
if __name__ == "__main__":
    ...
    httpd = ThreadingHTTPServer(server_address, DomainCheckerHandler)
